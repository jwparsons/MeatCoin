...
class Broadcast {
    ...
    static meatpotRules(channel) {
        const message = 'The meat bandit has stolen the Hope Turkey Leg and placed it in a mailbox in your neighborhood. Check each mailbox to find your prize, but beware of what else may be inside! 1 m¢/check.';
        channel.send(ttsFormat(message));
    }
    ...
    static meatPotFind(channel, trader, prize) {
        const message = 'Oh hell yeah, ' + trader.tag + 'got that ' + prize + 'better luck next time, losers. Hahaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.';
        channel.send(ttsFormat(message), {tts: true});
    }
