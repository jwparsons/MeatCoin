...
class Broadcast {
    ...
    static meatpotFind(channel, trader, prize) {
        const message = 'Oh heck yeah, ' + trader.name + ' got that ' + prize + ' better luck next time, losers. Hahahahahahahahahahahahahahaha.';
        channel.send(ttsFormat(message), {tts: true});
    }

    static meatpotMiss(channel, trader, prize) {
        const message = 'Dear idiot ' + '(' + trader.tag + '), I hope you like ' + prize + '. GROSS!\n\t- Meat Bandit';
        channel.send(standardFormat(message));
    }
}
